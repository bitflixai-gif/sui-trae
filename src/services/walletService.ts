...
export class WalletService {
  ...
  async debit(
    ...
    // Check balance first
    const balRes = await client.query(`select ${col} from wallets where user_id = $1`, [userId]);
    const currentBal = Number(balRes.rows[0]?.[col] ?? '0');
  ...
  async getBalances(client: PoolClient, userId: string) {
    await this.ensureWallet(client, userId);
    const res = await client.query(
      `select deposit_wallet, earnings_wallet, commission_wallet from wallets where user_id = $1`,
      [userId],
    );
    return res.rows[0];
  }
